import i18n from "i18next";
import { initReactI18next } from "react-i18next";

// Load language from localStorage or default to 'cs'
const savedLanguage = localStorage.getItem("i18nextLng") || "cs";

i18n.use(initReactI18next).init({
  resources: {
    en: {
      translation: {
        pages: {
          personalView: "Personal View",
          qsubAssembler: "Qsub Assembler",
          machines: "Machines",
          jobs: "Jobs",
          queues: "Jobs Queues",
          propertiesOfMachines: "Properties of Machines",
          users: "Users",
          resourceStatus: "Resource Status",
          storageSpaces: "Storage Spaces",
          waitingJobs: "Waiting Jobs",
          outages: "Outages",
          currentStatus: "Current Status",
          userSupport: "User Support",
          jobDetail: "Job Detail",
          machineDetail: "Machine Detail",
          clusterDetail: "Cluster Detail",
          queueDetail: "Queue Detail",
          userDetail: "User Detail",
          groups: "Groups",
          groupDetail: "Group Detail",
          projects: "Cloud Projects",
          projectDetail: "Project Detail",
          fairshare: "Fairshare",
          documentation: "Documentation",
          faq: "FAQ",
          metacentrum: "Metacentrum.cz",
        },
        queues: {
          routeQueue: "Route Queue",
          noAccess: "Access to this queue is reserved for authorized users",
          noAccessGroups:
            "Access to this queue is reserved for users in the following groups: {{groups}}",
          restrictedAccess: "This queue is restricted to specific groups",
          restrictedGroups:
            "This queue is restricted to users in the following groups: {{groups}}",
          reservedForUsers:
            "This queue is reserved for the following users: {{users}}",
          aclGroups: "Access Control Groups",
          aclUsers: "Reserved Users",
          noDirectSubmission:
            "Jobs cannot be submitted directly to this queue. Please use the route queue instead.",
          min: "Min",
          max: "Max",
          enabled: "Enabled",
          disabled: "Disabled",
          started: "Started",
          stopped: "Stopped",
          loading: "Loading queues...",
          errorLoading: "Error loading queues:",
          unknownError: "Unknown error",
          queueName: "Queue Name",
          priority: "Priority",
          timeLimits: "Time Limits",
          jobs: "Jobs",
          status: "Status",
          noQueuesFound: "No enabled and started queues found",
          noSearchResults: "No queues match your search",
          searchPlaceholder: "Search queues by name...",
          maxForUser: "Max/User",
          fairshare: "Fairshare",
          queued: "Queued:",
          held: "Held:",
          running: "Running:",
          done: "Done:",
          total: "Total:",
          basicInformation: "Basic Information",
          parentQueues: "Parent Queues",
          parentQueue: "Parent Queue",
          childrenQueues: "Children Queues",
          jobStatistics: "Job Statistics",
          noJobs: "No jobs found",
          noMachines: "No machines assigned to this queue",
          noSystemInfo: "No system information available",
          queueNotFound: "Queue not found",
          enabledAndStarted: "Enabled and Started",
          queueType: "Queue Type",
          minWalltime: "Minimum Walltime",
          maxWalltime: "Maximum Walltime",
          defaultWalltime: "Default Walltime",
          machineStatistics: "Machine Statistics",
          totalMachines: "Total Machines",
          reservation: "Reservation",
          reservationName: "Reservation Name",
          reservationOwner: "Reservation Owner",
          reservationStart: "Start Time",
          reservationEnd: "End Time",
          reservationResources: "Resources",
          reservationAuthorizedUsers: "Authorized Users",
          reservationNodes: "Reserved Nodes",
          reservationNotFound: "Reservation not found",
          reservationsTitle: "Reservations",
          queueReserved: "This queue is created for a reservation",
          queueReservedNotStarted:
            "This queue is created for a reservation but has not started yet",
          memory: "Memory",
          cpus: "CPUs",
          gpus: "GPUs",
          nodes: "Nodes",
          tabs: {
            jobs: "Jobs",
            machines: "Machines",
            systemInfo: "System Info",
          },
        },
        language: {
          english: "English",
          czech: "Čeština",
          switchLanguage: "Switch Language",
        },
        common: {
          logoAlt: "metacentrum cesnet",
          loading: "Loading...",
          errorLoading: "Error loading:",
          unknownError: "Unknown error",
          previous: "Previous",
          next: "Next",
          perPage: "Per page:",
          notLoggedIn: "You are not logged in",
          redirectingToLogin: "Redirecting to login...",
          logout: "Logout",
          toggleMenu: "Toggle menu",
          collapseMenu: "Collapse menu",
          expandMenu: "Expand menu",
          dataFreshness: {
            pbs: "PBS {{server}}",
            storageSpaces: "Storage spaces",
            storageQuotas: "Storage quotas",
            perun: "Perun",
            prometheus: "Prometheus",
            justNow: "just now",
            minutesAgo: "{{count}}m ago",
            hoursAgo: "{{count}}h ago",
            daysAgo: "{{count}}d ago",
            unknown: "n/a",
          },
        },
        jobs: {
          status: "Status",
          id: "ID",
          name: "Name",
          username: "User",
          machine: "Machine",
          cpuReserved: "CPU (reserved)",
          gpuReserved: "GPU (reserved)",
          ram: "RAM",
          created: "Created",
          searchPlaceholder: "Search by job ID, name, owner, or machine...",
          searchPlaceholderShort: "Search jobs...",
          totalJobs: "Total {{count}} jobs",
          totalJobsCompletedBy: "Total {{count}} jobs, completed by {{completedBy}}",
          noJobsFound: "No jobs found",
          exit: "Exit",
          cpuTime: "CPU time",
          gpuTime: "GPU time",
          of: "of",
          gb: "GB",
          anonym: "Anonym",
          state: {
            queued: "In queue",
            running: "Running",
            done: "Done",
            held: "Held",
            completed: "Completed",
            completed_with_error: "Completed",
            exiting: "Exiting",
            suspended: "Suspended",
            moved: "Moved",
            waiting: "Waiting",
            unknown: "Unknown",
          },
          all: "All",
          stateLabel: "State",
          basicInfo: "Basic Information",
          jobId: "Job ID",
          jobName: "Job Name",
          owner: "Owner",
          queue: "Queue",
          server: "Server",
          node: "Node",
          exitCode: "Exit Code",
          comment: "Comment",
          resources: "Resources",
          tabs: {
            myJobs: "My Jobs",
            allJobs: "All Jobs",
            waitingJobs: "Waiting Jobs",
          },
          requestedResources: "Requested Resources",
          noResourcesRequested: "No resources requested",
          resourceReservation: "Resource Reservation",
          resourceUsage: "Resource Usage",
          cpu: "CPU",
          gpu: "GPU",
          noGpu: "no GPU",
          memory: "Memory",
          scratch: "Scratch",
          walltime: "Walltime",
          cpuTimeUsed: "CPU Time Used",
          gpuTimeUsed: "GPU Time Used",
          memoryUsed: "Memory Used",
          runtime: "Runtime",
          allocatedResources: "Allocated Resources",
          subjobs: "Subjobs",
          subjobId: "Subjob ID",
          parentJob: "Parent job",
          parentJobTooltip:
            "This is a parent job (controller job) for an array job",
          systemInformation: "System Information",
          timestamps: "Timestamps",
          createdAt: "Created",
          eligibleAt: "Eligible",
          estimatedStartTime: "Estimated Start",
          estimatedNodes: "Estimated Resources",
          startedAt: "Started",
          completedBy: "Completed by",
          obitAt: "Completed",
          lastStateChangeAt: "Last State Change",
          kerberosTicketAt: "Kerberos Ticket",
          directories: "Directories",
          stdoutDirectory: "Stdout Directory",
          workingDirectory: "Working Directory",
          scratchDirectory: "Scratch Directory",
          environmentVariables: "Environment Variables",
          messages: "Messages",
          jobNotFound: "Job not found",
          messageTypes: {
            cpuUsageLow:
              "CPU usage is below 75% ({{percent}}%). The job was inefficient. Consider reducing the number of CPUs requested.",
            gpuUsageLow:
              "GPU usage is below 75% ({{percent}}%). Consider reducing the number of GPUs requested.",
            memoryUsageLow:
              "Memory usage is below 75% ({{percent}}%). Consider reducing the amount of memory requested.",
            memoryOverAllocated:
              "Memory usage ({{memoryUsed}} GB) exceeds reserved memory ({{memoryReserved}} GB).",
            walltimeTooHigh:
              "Job finished way sooner than requested. Actual walltime: {{walltimeUsed}}, Reserved: {{walltimeReserved}}. Consider reducing the requested walltime.",
          },
        },
        users: {
          description: "In the user detail, you can find several pieces of information.  Click the user button to see: membership expiration, publications and acknowledgments, group affiliations, your fairshare, usage statistics, and more.",
          username: "Username",
          nickname: "Nickname",
          fullName: "Full Name",
          tasks: "Tasks",
          cpuTasks: "CPU Tasks",
          fairsharePerServer: "Fairshare per Server",
          fairsharePriority: "Priority",
          server: "Server",
          transit: "Transit",
          held: "Held",
          waiting: "Waiting",
          exiting: "Exiting",
          noFairshareData: "No fairshare data available",
          usersBeforeYou: "{{count}} users have priority before you",
          noUsersBeforeYou: "No users have priority before you",
          userNotFound: "User not found",
          totalTasks: "Total Tasks",
          runningTasks: "Running Tasks",
          resourceUsage: "Resource Usage",
          noJobs: "No jobs",
          noResources: "No resources",
          noCpuResources: "no CPU resources",
          noGpuResources: "no GPU resources",
          noUsersFound: "No users found",
          actions: "Actions",
          impersonate: "Impersonate",
          impersonating: "Impersonating: {{username}}",
          cancelImpersonation: "Cancel",
          searchPlaceholder: "Search by username or nickname...",
          basicInfo: "Basic Information",
          organization: "Organization",
          mail: "Preferred e-mail",
          lang: "Preferred language",
          publications: "Publications",
          groups: "Groups",
          membershipExpiration: "Membership Expiration",
          tabs: {
            jobs: "Jobs",
            queues: "Queues",
          },
          totalUsers: "Total {{count}} users",
          totalUsersFiltered: "{{filtered}} of {{total}} users",
          accounting: {
            title: "Accounting Information",
            notAvailable: "Accounting data is not available",
            totalJobs: "Total Jobs",
            totalCpuTime: "Total CPU Time",
            yearlyUsage: "Yearly Usage Breakdown",
            year: "Year",
            jobs: "Jobs",
            cpuTime: "CPU Time",
            year1: "year",
            years4: "years",
            years5plus: "years",
            day1: "day",
            days4: "days",
            days5plus: "days",
          },
        },
        groups: {
          name: "Name",
          gid: "GID",
          memberCount: "Members",
          members: "Members",
          nickname: "Nickname",
          groupNotFound: "Group not found",
        },
        fairshare: {
          ranking: "Ranking",
          username: "Username",
          noData: "No fairshare data available",
          noEntries: "No entries found",
          infoTooltip:
            "Fairshare value indicates your priority in the queue. The higher the value, the better the priority. The ranking is calculated per scheduler server based on your fairshare value.",
        },
        machines: {
          metacentrumOverview: "MetaCentrum Overview",
          totalOrganizations: "Total Organizations",
          totalClusters: "Total Clusters",
          totalNodes: "Total Nodes",
          totalCpu: "Total CPU",
          totalGpu: "Total GPU",
          freeNodes: "Free Nodes",
          partiallyUsedNodes: "Partially Used Nodes",
          usedNodes: "Used Nodes",
          unknownNodes: "Unknown Nodes",
          searchPlaceholder: "Search by name or queue...",
          searchTotalMachines: "Total {{count}} nodes",
          searchFilterPlaceholder: "Search...",
          quickLinks: "Quick Links",
          nodeName: "Node Name",
          state: "State",
          queue: "Queue",
          cluster: "Cluster",
          resource: "Resource",
          all: "All",
          resourceUsage: "Resource Usage",
          reservedResources: "Reserved Resources",
          gpuDetails: "GPU Details",
          gpuCapability: "GPU Capability",
          gpuMemory: "GPU Memory",
          cudaVersion: "CUDA Version",
          systemInformation: "System Information",
          totalMemory: "Total Memory",
          nodeNotFound: "Node not found",
          basicInformation: "Basic Information",
          tasksTabPlaceholder: "Tasks will be displayed here",
          queuesTabPlaceholder: "Queues will be displayed here",
          outagesTabPlaceholder:
            "Operational restrictions will be displayed here",
          noJobs: "No jobs running on this node",
          noQueues: "No queues available for this node",
          noSystemInfo: "No system information available",
          noOutages: "No operational restrictions",
          attribute: "Attribute",
          value: "Value",
          nodeStateLegend: "Node states",
          tabs: {
            tasks: "Tasks",
            queues: "Queues",
            systemInfo: "System Information",
            outages: "Operational Restrictions",
          },
          nodeState: {
            free: "Free",
            partiallyUsed: "Partially Free",
            used: "Occupied",
            unknown: "Unknown",
            maintenance: "Maintenance",
            notAvailable: "Not Available",
          },
          errorMessages: "Error Messages",
          clusterName: "Cluster Name",
          memory: "Memory",
          specifications: "Specifications",
          scratchSpace: "Scratch Space",
          scratchLocal: "Scratch Local",
          scratchSsd: "Scratch SSD",
          scratchShared: "Scratch Shared",
          scratchShm: "Scratch SHM",
          available: "available",
          description: "Description",
          specification: "Specification",
          cpuDescription: "CPU Description",
          gpuDescription: "GPU Description",
          disk: "Disk",
          network: "Network",
          comment: "Comment",
          owner: "Owner",
          virtualOrganizations: "Virtual Organizations",
          clusterStatistics: "Cluster Statistics",
          noPbs: "No PBS",
          nodes: "Nodes",
          clusterNotFound: "Cluster not found",
          cloudNode: "Cloud Node Information",
          cloudCpu: "Cloud CPU Count",
          vmCount: "VM Count",
          cpuModel: "CPU Model",
          reservation: "Reservation",
          reservationName: "Reservation Name",
          reservationOwner: "Reservation Owner",
          reservationStart: "Start Time",
          reservationEnd: "End Time",
          reservationResources: "Resources",
          reservationAuthorizedUsers: "Authorized Users",
          reservationQueue: "Queue",
          reservationStarted: "Started",
          reservationNotStarted: "Not Started",
          overcommittedReason: "This node is overcommitted:",
          overcommittedReasonResJob: "There are reservations and running jobs whose combined resource requirements exceed the node's capacity.",
        },
        storageSpaces: {
          scratchStorage: {
            title: "Scratch Storage",
            description:
              "Each compute node is equipped with fast local data storage. You can find the free space in the detail of each node.",
            warning:
              "Use scratch storage only during computation. After the job ends, data from scratch is deleted after 14 days.",
          },
          diskArrays: {
            title: "Disk Arrays",
            description:
              "Disk arrays are made up of many connected disks. One file can be stored on multiple disks, and data is stored multiple times. Compared to a single disk, the array is faster for both reading and writing, and has greater resistance to data loss. Use disk arrays to store data used between jobs.",
          },
          table: {
            directory: "Directory",
            usage: "Usage - Used Free",
            size: "Size",
            freeSpace: "Free space",
          },
          totals: {
            total: "Total storage space in MetaCentrum",
            used: "Total used",
            free: "Total free space",
          },
          userQuotas: {
            title: "My Storage Quotas",
            username: "User",
            noData: "No quota data available",
            directory: "Directory",
            used: "Used",
            softQuota: "Soft Quota",
            hardQuota: "Hard Quota",
            grace: "Grace",
            filesUsed: "Files Used",
            filesLimit: "Files Limit (soft/hard)",
            filesGrace: "Files Grace",
          },
        },
        projects: {
          name: "Name",
          id: "ID",
          description: "Description",
          type: "Type",
          personal: "Personal",
          personalTooltip:
            "This is a personal project that belongs only to you. You are the only member.",
          myProject: "My Project",
          shared: "Shared",
          noDescription: "No description",
          noProjects: "No projects found",
          status: "Status",
          statusActive: "Active",
          statusExpired: "Expired",
          reservedResources: "Reserved Resources",
          createdAt: "Creation Date",
          vms: "VMs",
          vcpus: "vCPUs",
          memory: "Memory",
          noDate: "N/A",
          searchPlaceholder: "Search by name, description, or ID...",
          totalProjects: "Total: {{count}} projects",
          basicInfo: "Basic Information",
          vmName: "VM Name",
          vmId: "VM ID",
          flavor: "Flavor",
          noVms: "No VMs found in this project",
          projectNotFound: "Project not found",
          users: "Users",
          noUsers: "No users found in this project",
          userName: "Name",
          userLogname: "Login Name",
          userOrg: "Organization",
          userNotFoundInPerun: "User with this ID was not found in Perun data",
        },
        personalView: {
          totalStatistics: "Total Statistics",
          myStatistics: "My Statistics",
          totalTasks: "Total Tasks",
          resourceUtilization: "Resource Utilization",
          totalActualJobs: "Total Actual Jobs",
          totalCPUUsage: "Total CPU Usage",
          totalGPUUsage: "Total GPU Usage",
          myPbsJobs: "My PBS Jobs",
          myCloudProjects: "My Cloud Projects",
          noGpuResources: "no GPU resources",
        },
      },
    },
    cs: {
      translation: {
        pages: {
          personalView: "Můj přehled",
          qsubAssembler: "Sestavovač úloh (qsub)",
          machines: "Stroje MetaCentra",
          jobs: "Úlohy",
          queues: "Fronty úloh",
          propertiesOfMachines: "Vlastnosti strojů",
          users: "Uživatelé mých skupin",
          resourceStatus: "Stav zdrojů",
          storageSpaces: "Úložné prostory",
          waitingJobs: "Čekající úlohy",
          outages: "Výpadky",
          currentStatus: "Aktualní stav",
          userSupport: "Uživatelská podpora",
          jobDetail: "Detail úlohy",
          machineDetail: "Detail stroje",
          clusterDetail: "Detail clusteru",
          queueDetail: "Detail fronty",
          userDetail: "Detail uživatele",
          groups: "Skupiny",
          groupDetail: "Detail skupiny",
          projects: "Projekty v cloudu",
          projectDetail: "Detail projektu",
          fairshare: "Fairshare",
          documentation: "Dokumentace",
          faq: "FAQ",
          metacentrum: "Metacentrum.cz",
        },
        queues: {
          routeQueue: "Směrovací fronta",
          noAccess:
            "Přístup k této frontě je vyhrazen autorizovaným uživatelům",
          noAccessGroups:
            "Přístup k této frontě je vyhrazen uživatelům v následujících skupinách: {{groups}}",
          restrictedAccess: "Tato fronta je omezena na specifické skupiny",
          restrictedGroups:
            "Tato fronta je omezena na uživatele v následujících skupinách: {{groups}}",
          reservedForUsers:
            "Tato fronta je vyhrazena pro následující uživatele: {{users}}",
          aclGroups: "Skupiny s přístupem",
          aclUsers: "Vyhrazení uživatelé",
          noDirectSubmission:
            "Úlohy nelze odeslat přímo do této fronty. Použijte prosím směrovací frontu.",
          min: "Min",
          max: "Max",
          enabled: "Povoleno",
          disabled: "Zakázáno",
          started: "Spuštěno",
          stopped: "Zastaveno",
          loading: "Načítání front...",
          errorLoading: "Chyba při načítání front:",
          unknownError: "Neznámá chyba",
          queueName: "Název fronty",
          priority: "Priorita",
          timeLimits: "Časové limity",
          jobs: "Úlohy",
          status: "Stav",
          noQueuesFound: "Nebyly nalezeny žádné povolené a spuštěné fronty",
          noSearchResults: "Vašemu hledání neodpovídá žádná fronta",
          searchPlaceholder: "Hledat fronty podle názvu...",
          maxForUser: "Max na uživatele",
          fairshare: "Fairshare",
          queued: "Ve frontě:",
          held: "Zadrženo:",
          running: "Běží:",
          done: "Hotovo:",
          total: "Celkem:",
          basicInformation: "Základní informace",
          parentQueues: "Nadřazené fronty",
          parentQueue: "Nadřazená fronta",
          childrenQueues: "Podřízené fronty",
          jobStatistics: "Statistiky úloh",
          noJobs: "Nebyly nalezeny žádné úlohy",
          noMachines: "K této frontě nejsou přiřazeny žádné stroje",
          noSystemInfo: "Nejsou k dispozici žádné systémové informace",
          queueNotFound: "Fronta nenalezena",
          enabledAndStarted: "Povoleno a spuštěno",
          queueType: "Typ fronty",
          minWalltime: "Minimální walltime",
          maxWalltime: "Maximální walltime",
          defaultWalltime: "Výchozí walltime",
          machineStatistics: "Statistiky strojů",
          totalMachines: "Celkem strojů",
          reservation: "Rezervace",
          reservationName: "Název rezervace",
          reservationOwner: "Vlastník rezervace",
          reservationStart: "Čas začátku",
          reservationEnd: "Čas konce",
          reservationResources: "Prostředky",
          reservationAuthorizedUsers: "Autorizovaní uživatelé",
          reservationNodes: "Rezervované uzly",
          reservationNotFound: "Rezervace nenalezena",
          reservationsTitle: "Rezervace",
          queueReserved: "Tato fronta je vytvořena pro rezervaci",
          queueReservedNotStarted:
            "Tato fronta je vytvořena pro rezervaci, ale ještě nebyla spuštěna",
          memory: "Paměť",
          cpus: "CPU",
          gpus: "GPU",
          nodes: "Uzly",
          tabs: {
            jobs: "Úlohy",
            machines: "Stroje",
            systemInfo: "Systémové informace",
          },
        },
        language: {
          english: "English",
          czech: "Čeština",
          switchLanguage: "Přepnout jazyk",
        },
        common: {
          logoAlt: "metacentrum cesnet",
          loading: "Načítání...",
          errorLoading: "Chyba při načítání:",
          unknownError: "Neznámá chyba",
          previous: "Předchozí",
          next: "Další",
          perPage: "Na stránku:",
          notLoggedIn: "Nejste přihlášeni",
          redirectingToLogin: "Přesměrovávání na přihlášení...",
          logout: "Odhlásit se",
          toggleMenu: "Přepnout menu",
          collapseMenu: "Sbalit menu",
          expandMenu: "Rozbalit menu",
          dataFreshness: {
            pbs: "PBS {{server}}",
            storageSpaces: "Diskové prostory",
            storageQuotas: "Diskové kvóty",
            perun: "Perun",
            prometheus: "Prometheus",
            justNow: "právě teď",
            minutesAgo: "před {{count}} min",
            hoursAgo: "před {{count}} h",
            daysAgo: "před {{count}} d",
            unknown: "neznámo",
          },
        },
        jobs: {
          status: "Stav",
          id: "ID",
          name: "Název",
          username: "Uživatel",
          machine: "Stroj",
          cpuReserved: "CPU (vyhrazené)",
          gpuReserved: "GPU (vyhrazené)",
          ram: "RAM",
          created: "Vytvořené",
          searchPlaceholder:
            "Hledat podle ID úlohy, názvu, vlastníka nebo stroje...",
          searchPlaceholderShort: "Hledat úlohy...",
          totalJobs: "Celkem {{count}} úloh",
          totalJobsCompletedBy: "Celkem {{count}} úloh, skončí do {{completedBy}}",
          noJobsFound: "Nebyly nalezeny žádné úlohy",
          exit: "Exit",
          cpuTime: "CPU čas",
          gpuTime: "GPU čas",
          of: "z",
          gb: "GB",
          anonym: "Anonym",
          state: {
            queued: "Ve frontě",
            running: "Běží",
            done: "Hotovo",
            held: "Zadrženo",
            completed: "Hotovo",
            completed_with_error: "Hotovo",
            exiting: "Končí",
            suspended: "Pozastaveno",
            moved: "Přesunuto",
            waiting: "Čeká",
            unknown: "Neznámý",
          },
          all: "Vše",
          stateLabel: "Stav",
          basicInfo: "Základní informace",
          jobId: "ID úlohy",
          jobName: "Název úlohy",
          owner: "Vlastník",
          queue: "Fronta",
          server: "Server",
          node: "Uzel",
          exitCode: "Návratový kód",
          comment: "Komentář",
          waitingReason: "Důvod čekání",
          waitingJobsSummary: "Přehled úloh podle důvodu čekání",
          filterByReason: "Filtrovat podle důvodu",
          clearFilter: "Zrušit filtr",
          filter: "Filtrovat",
          clear: "Zrušit",
          resources: "Prostředky",
          tabs: {
            myJobs: "Moje úlohy",
            allJobs: "Všechny úlohy",
            waitingJobs: "Čekající úlohy",
          },
          requestedResources: "Požadované prostředky",
          noResourcesRequested: "Žádné prostředky nebyly požadovány",
          resourceReservation: "Vyhrazení prostředků",
          resourceUsage: "Využití prostředků",
          cpu: "CPU",
          gpu: "GPU",
          noGpu: "žádné GPU",
          memory: "Paměť",
          scratch: "Scratch",
          walltime: "Walltime",
          cpuTimeUsed: "Použitý CPU čas",
          gpuTimeUsed: "Použitý GPU čas",
          memoryUsed: "Použitá paměť",
          runtime: "Čas běhu",
          allocatedResources: "Přidělené prostředky",
          subjobs: "Podúlohy",
          subjobId: "ID podúlohy",
          parentJob: "Řidici uloha",
          parentJobTooltip: "Toto je řídicí úloha (parent job) pro pole úloh",
          systemInformation: "Systémové informace",
          timestamps: "Časové značky",
          createdAt: "Vytvořeno",
          eligibleAt: "Způsobilé k běhu",
          estimatedStartTime: "Odhad spuštění",
          estimatedNodes: "Odhadované prostředky",
          startedAt: "Spuštěno",
          completedBy: "Skončí do",
          obitAt: "Dokončeno",
          lastStateChangeAt: "Poslední změna stavu",
          kerberosTicketAt: "Kerberos lístek",
          directories: "Adresáře",
          stdoutDirectory: "Adresář pro stdout",
          workingDirectory: "Pracovní adresář",
          scratchDirectory: "SCRATCHDIR",
          environmentVariables: "Seznam proměnných",
          messages: "Zprávy",
          jobNotFound: "Úloha nenalezena",
          messageTypes: {
            cpuUsageLow:
              "Využití CPU je pod 75% ({{percent}}%). Úloha byla neefektivní. Zvažte snížení počtu požadovaných CPU.",
            gpuUsageLow:
              "Využití GPU je pod 75% ({{percent}}%). Zvažte snížení počtu požadovaných GPU.",
            memoryUsageLow:
              "Využití paměti je pod 75% ({{percent}}%). Zvažte snížení množství požadované paměti.",
            memoryOverAllocated:
              "Využití paměti ({{memoryUsed}} GB) překračuje vyhrazenou paměť ({{memoryReserved}} GB).",
            walltimeTooHigh:
              "Úloha skončila výrazně dříve, než bylo požadováno. Skutečný walltime: {{walltimeUsed}}, Vyhrazený: {{walltimeReserved}}. Zvažte snížení požadovaného walltime.",
          },
        },
        users: {
          description: "V detailech uživatele najdete několik informací. Kliknutím na tlačítko uživatele zobrazíte: datum vypršení členství, publikace a poděkování, příslušnost ke skupinám, fairshare, statistiky používání a další.",
          username: "Uživatelské jméno",
          nickname: "Přezdívka",
          fullName: "Celé jméno",
          tasks: "Úlohy",
          cpuTasks: "CPU úlohy",
          fairsharePerServer: "Fairshare podle serveru",
          fairsharePriority: "Priorita",
          server: "Server",
          transit: "Transit",
          held: "Zadrženo",
          waiting: "Čeká",
          exiting: "Končí",
          noFairshareData: "Nejsou k dispozici žádná fairshare data",
          usersBeforeYou: "{{count}} uživatelů má před vámi prioritu",
          noUsersBeforeYou: "Žádný uživatel nemá před vámi prioritu",
          userNotFound: "Uživatel nenalezen",
          totalTasks: "Celkem úloh",
          runningTasks: "Běžící úlohy",
          resourceUsage: "Využití zdrojů",
          noJobs: "Žádné úlohy",
          noResources: "Žádné zdroje",
          noCpuResources: "žádné CPU zdroje",
          noGpuResources: "žádné GPU zdroje",
          noUsersFound: "Nebyli nalezeni žádní uživatelé",
          actions: "Akce",
          impersonate: "Zosobnit",
          impersonating: "Zosobňování: {{username}}",
          cancelImpersonation: "Zrušit",
          searchPlaceholder:
            "Hledat podle uživatelského jména nebo přezdívky...",
          basicInfo: "Základní informace",
          organization: "Organizace",
          mail: "Preferovaný e-mail",
          lang: "Preferovaný jazyk",
          publications: "Publikace",
          groups: "Skupiny",
          membershipExpiration: "Datum vypršení členství",
          tabs: {
            jobs: "Úlohy",
            queues: "Fronty",
          },
          totalUsers: "Celkem {{count}} uživatelů",
          totalUsersFiltered: "{{filtered}} z {{total}} uživatelů",
          accounting: {
            title: "Účetní informace",
            notAvailable: "Účetní data nejsou k dispozici",
            totalJobs: "Celkem úloh",
            totalCpuTime: "Celkový CPU čas",
            yearlyUsage: "Roční přehled využití",
            year: "Rok",
            jobs: "Úlohy",
            cpuTime: "CPU čas",
            year1: "rok",
            years4: "roky",
            years5plus: "let",
            day1: "den",
            days4: "dny",
            days5plus: "dnů",
          },
        },
        groups: {
          name: "Název",
          gid: "GID",
          memberCount: "Členové",
          members: "Členové",
          nickname: "Přezdívka",
          groupNotFound: "Skupina nenalezena",
        },
        fairshare: {
          ranking: "Pořadí",
          username: "Uživatelské jméno",
          noData: "Nejsou k dispozici žádná fairshare data",
          noEntries: "Nebyly nalezeny žádné záznamy",
          infoTooltip:
            "Fairshare hodnota ukazuje prioritu vašich úloh ve frontě. Čím vyšší hodnota, tím lepší priorita. Pořadí se počítá pro každý plánovač zvlášť na základě vaší fairshare hodnoty.",
        },
        machines: {
          metacentrumOverview: "Přehled MetaCentrum",
          totalOrganizations: "Celkem organizací",
          totalClusters: "Celkem clusterů",
          totalNodes: "Celkem uzlů",
          totalCpu: "Celkem CPU",
          totalGpu: "Celkem GPU",
          freeNodes: "Volné uzly",
          partiallyUsedNodes: "Částečně použité uzly",
          usedNodes: "Použité uzly",
          unknownNodes: "Neznámé uzly",
          searchPlaceholder: "Hledat podle názvu nebo fronty...",
          searchTotalMachines: "Celkem {{count}} strojů",
          searchFilterPlaceholder: "Hledat...",
          quickLinks: "Rychlé odkazy",
          nodeName: "Název uzlu",
          state: "Stav",
          queue: "Fronta",
          cluster: "Cluster",
          resource: "Zdroj",
          all: "Vše",
          resourceUsage: "Využití zdrojů",
          reservedResources: "Vyhrazené zdroje",
          gpuDetails: "Detaily GPU",
          gpuCapability: "Schopnost GPU",
          gpuMemory: "Paměť GPU",
          cudaVersion: "Verze CUDA",
          systemInformation: "Systémové informace",
          totalMemory: "Celková paměť",
          nodeNotFound: "Uzel nenalezen",
          basicInformation: "Základní informace",
          tasksTabPlaceholder: "Úlohy budou zobrazeny zde",
          queuesTabPlaceholder: "Fronty budou zobrazeny zde",
          outagesTabPlaceholder:
            "Zaznamenaná omezení běžného provozu budou zobrazena zde",
          noJobs: "Na tomto uzlu neběží žádné úlohy",
          noQueues: "Pro tento uzel nejsou k dispozici žádné fronty",
          noSystemInfo: "Nejsou k dispozici žádné systémové informace",
          noOutages: "Žádná omezení běžného provozu",
          attribute: "Vlastnost",
          value: "Hodnota",
          tabs: {
            tasks: "Úlohy",
            queues: "Fronty",
            systemInfo: "Systémové informace",
            outages: "Zaznamenaná omezení běžného provozu",
          },
          nodeStateLegend: "Stavy uzlů",
          nodeState: {
            free: "Volný",
            partiallyUsed: "Částečně volný",
            used: "Obsazený",
            unknown: "Neznámý",
            maintenance: "Údržba",
            notAvailable: "Nedostupný",
          },
          errorMessages: "Chybové zprávy",
          clusterName: "Název clusteru",
          memory: "Paměť",
          specifications: "Specifikace",
          scratchSpace: "Scratch úložiště",
          scratchLocal: "Scratch Local",
          scratchSsd: "Scratch SSD",
          scratchShared: "Scratch Shared",
          scratchShm: "Scratch SHM",
          available: "dostupné",
          description: "Popis",
          specification: "Specifikace",
          cpuDescription: "Popis CPU",
          gpuDescription: "Popis GPU",
          disk: "Disk",
          network: "Síť",
          comment: "Komentář",
          owner: "Vlastník",
          virtualOrganizations: "Virtuální organizace",
          clusterStatistics: "Statistiky clusteru",
          noPbs: "Bez PBS",
          nodes: "Uzly",
          clusterNotFound: "Cluster nenalezen",
          cloudNode: "Informace o cloudovém uzlu",
          cloudCpu: "Počet CPU v cloudu",
          vmCount: "Počet VM",
          cpuModel: "Model CPU",
          reservation: "Rezervace",
          reservationName: "Název rezervace",
          reservationOwner: "Vlastník rezervace",
          reservationStart: "Čas začátku",
          reservationEnd: "Čas konce",
          reservationResources: "Prostředky",
          reservationAuthorizedUsers: "Autorizovaní uživatelé",
          reservationQueue: "Fronta",
          reservationStarted: "Spuštěno",
          reservationNotStarted: "Nespuštěno",
          overcommittedReason: "Tento uzel je přetížen:",
          overcommittedReasonResJob: "Probíhají rezervace a běží úlohy, jejichž kombinované požadavky na prostředky překračují kapacitu uzlu.",
        },
        storageSpaces: {
          scratchStorage: {
            title: "Úložiště scratch",
            description:
              "Každý výpočetní uzel je vybaven rychlým lokálním úložištěm dat, velikost volného místa zjistíte v detailu každého uzlu.",
            warning:
              "Úložiště scratch používejte pouze po dobu výpočtu. Po skončení úlohy jsou data ze scratche po 14 dnech smazána.",
          },
          diskArrays: {
            title: "Disková pole",
            description:
              "Disková pole jsou tvořena mnoha spojenými disky. Jeden soubor může být uložen na více discích, navíc data jsou ukládána vícenásobně. Ve srovnání s jednotlivým diskem je proto pole rychlejší při čtení i zápisu, a má větší odolnost proti ztrátě dat. Disková pole používejte k ukládání používaných dat v době mezi úlohami.",
          },
          table: {
            directory: "adresář",
            usage: "využití - využito volné",
            size: "velikost",
            freeSpace: "volné místo",
          },
          totals: {
            total: "Celkem úložného místa v MetaCentru",
            used: "Celkem využito",
            free: "Celkem volného místa",
          },
          userQuotas: {
            title: "Moje kvóty úložiště",
            username: "Uživatel",
            noData: "Žádná data o kvótách nejsou k dispozici",
            directory: "Adresář",
            used: "Využito",
            softQuota: "Měkká kvóta",
            hardQuota: "Tvrdá kvóta",
            grace: "Ochranná lhůta",
            filesUsed: "Počet souborů",
            filesLimit: "Limit souborů (měkký/tvrdý)",
            filesGrace: "Ochranná lhůta",
          },
        },
        projects: {
          name: "Název",
          id: "ID",
          description: "Popis",
          type: "Typ",
          personal: "Osobní",
          personalTooltip:
            "Toto je osobní projekt, který patří pouze vám. Jste jediný člen.",
          myProject: "Můj projekt",
          shared: "Sdílený",
          noDescription: "Bez popisu",
          noProjects: "Nebyly nalezeny žádné projekty",
          status: "Stav",
          statusActive: "Aktivní",
          statusExpired: "Expirovaný",
          reservedResources: "Vyhrazené prostředky",
          createdAt: "Datum vytvoření",
          vms: "VM",
          vcpus: "vCPU",
          memory: "Paměť",
          noDate: "N/A",
          searchPlaceholder: "Hledat podle názvu, popisu nebo ID...",
          totalProjects: "Celkem: {{count}} projektů",
          basicInfo: "Základní informace",
          vmName: "Název VM",
          vmId: "ID VM",
          flavor: "Flavor",
          noVms: "V tomto projektu nebyly nalezeny žádné VM",
          projectNotFound: "Projekt nenalezen",
          users: "Uživatelé",
          noUsers: "V tomto projektu nebyli nalezeni žádní uživatelé",
          userName: "Jméno",
          userLogname: "Přihlašovací jméno",
          userOrg: "Organizace",
          userNotFoundInPerun:
            "Uživatel s tímto ID nebyl nalezen v Perun datech",
        },
        personalView: {
          totalStatistics: "Celkové statistiky",
          myStatistics: "Moje statistiky",
          totalTasks: "Celkem úloh",
          resourceUtilization: "Využití zdrojů",
          totalActualJobs: "Celkový počet skutečných úloh",
          totalCPUUsage: "Celkové využití CPU",
          totalGPUUsage: "Celkové využití GPU",
          myPbsJobs: "Moje PBS úlohy",
          myCloudProjects: "Moje cloudové projekty",
          noGpuResources: "žádné GPU zdroje",
        },
      },
    },
  },
  lng: savedLanguage,
  fallbackLng: "cs",
  interpolation: { escapeValue: false },
});

// Persist language changes to localStorage
i18n.on("languageChanged", (lng) => {
  localStorage.setItem("i18nextLng", lng);
});
